...
-- Prefers YBA 1.7974+ internal dialogue-state selling via setupvalue; falls back to DialogueGui automation.
-- =====================
...
if type(_rawSetupvalue) ~= "function" and debug then
    _rawSetupvalue = debug.setupvalue
end
...
local function waitForDialogueState(previousSignature, timeout)
    local deadline = tick() + (timeout or 2.5)
    while tick() < deadline do
        local dlg = getDialogueGui()
        if dlg then suppressDialogueRendering() end

        local sig, buttons = getDialogueStateSignature()
        if sig ~= "" and sig ~= previousSignature and #buttons > 0 then
            return sig, buttons
        end
        task.wait(0.01)
    end
...
-- Proven on current YBA 1.7974+ / Delta:
--   Option1 (sell) -> Option1 (deal) -> Option6 (sell all)
-- The GUI is hidden immediately when it is created.  The first dialogue page
-- still has to initialize internally, but no visible clicking/navigation occurs.
local function tryInternalMerchantSell(itemName, merchantPrompt)
    ...
    if not tool then
        return false, 0, "could not equip item"
    end
    ...
    if not opened then
        return false, 0, "Merchant prompt failed"
    end

    -- Hide the initial dialogue as soon as it exists, before the first option
    -- page becomes ready.  This removes the visible intro wait for the user.
    ...
    for stage, optionName in ipairs(desired) do
        local sig = select(1, waitForDialogueState(previousSignature, stage == 1 and 3.0 or 1.2))
        if not sig then
            restoreDialogueRendering()
            return false, 0, "stage " .. tostring(stage) .. " timed out"
        end

        moduleLog("INFO", "[Inventory][InternalSell] Stage " .. tostring(stage)
            .. " -> " .. optionName .. " | " .. sig)
...
function Inventory:SellAll()
    ...
    local fallbackFastMode = fastRequested and hasInternalClickSupport()

    if fastRequested and internalSupported then
        moduleLog("INFO", "[Inventory][InternalSell] setupvalue route available — using hidden Option1 > Option1 > Option6.")
    elseif fastRequested then
        moduleLog("WARN", "[Inventory][InternalSell] setupvalue route unavailable — using dialogue click fallback.")
    end
    ...
    for _, itemName in ipairs(toSell) do
        ...
        local internalWorked = false

        if fastRequested and internalSupported and merchantPrompt then
            local ok, sold, reason = tryInternalMerchantSell(itemName, merchantPrompt)
            finalCount = self:Count(itemName)
            internalWorked = ok and sold > 0

            if internalWorked then
                moduleLog("INFO", "[Inventory][InternalSell] ✅ Sold " .. tostring(sold) .. "x " .. itemName
                    .. " through hidden internal dialogue state (remaining: " .. tostring(finalCount) .. ").")
            else
                moduleLog("WARN", "[Inventory][InternalSell] No confirmed progress for " .. itemName
                    .. " — " .. tostring(reason or "unknown reason") .. ".")
            end
        end

        -- If the internal state route is unavailable or failed, retain the
        -- adaptive dialogue driver as compatibility fallback.  We intentionally
        -- skip the obsolete pre-1.7974 EndDialogue packet here.
