-- Build patch: R15-EXACT-TEST-ALL
-- =====================
-- Inventory.lua
-- BUILD: ZERO-DELAY-2026.09.14-R7-EXACT-TEST-ALL
...
-- Exact production port of DialogueAnalyzer R2's TEST ALL logic.  This is
-- intentionally simple: inspect the CURRENT DialogueGui stage, hide it locally,
-- then try the same executor-side routes in the same order the analyzer proved
-- on Delta.  Quest code can call this when a prompt opens a dialogue whose final
-- page is an option-less ClickContinue callback (ClientFunctions:2088).
local function testAllStageToken(gui)
    if not gui or not gui.Parent then return "<closed>" end
    local sig = select(1, getDialogueStateSignature())
    local click = findClickContinue(gui)
    local connCount = 0
    if click and type(_rawGetConnections) == "function" then
        local signal = nil
...
local function testAllStageChanged(gui, before)
    task.wait(0.035)
    local current = getDialogueGui()
    if not current or current ~= gui or not gui.Parent then return true end
    return testAllStageToken(gui) ~= before
end

local function testAllHideDialogue(gui)
...
local function testAllOption1(gui, wantedName)
    wantedName = tostring(wantedName or "Option1")
    local option = findDialogueButtonByOptionName(wantedName)
    if not option then return false, "no " .. wantedName end

    local signal = nil
    pcall(function() signal = option.MouseButton1Click end)
    if not signal then pcall(function() signal = option.Activated end) end
    if not signal then return false, wantedName .. " has no signal" end
    ...
    if type(_rawGetConnections) == "function" then
        ...
        if ok and type(conns) == "table" then
            for _, conn in pairs(conns) do
                ...
                pcall(function() fn = conn.Function end)
                local _, line = functionSourceLine(fn)
                local before = testAllStageToken(gui)

                -- Same first test as DialogueAnalyzer R2.
                if type(_rawSetupvalue) == "function" and type(fn) == "function" then
                    local okSet = pcall(_rawSetupvalue, fn, 1, wantedName)
                    if okSet and testAllStageChanged(gui, before) then
                        return true, "setupvalue line=" .. tostring(line)
                    end
                end

                before = testAllStageToken(gui)
                ...
                if type(fireMethod) == "function" then
                    ...
                    if okFire and testAllStageChanged(gui, before) then
                        return true, "connection:Fire line=" .. tostring(line)
                    end
                end
                ...
                if type(fn) == "function" then
                    local okFn = pcall(fn)
                    if okFn and testAllStageChanged(gui, before) then
                        return true, "connection.Function line=" .. tostring(line)
                    end
                end
            end
    ...
    if type(_rawFireSignal) == "function" then
        local before = testAllStageToken(gui)
        local okFire = pcall(_rawFireSignal, signal)
        if okFire and testAllStageChanged(gui, before) then
            return true, "firesignal"
        end
    end
    return false, "all " .. wantedName .. " methods produced no stage change"
end

local function testAllClickContinue(gui)
    local click = findClickContinue(gui)
...
-- Production equivalent of DialogueAnalyzer's TEST ALL button.  Unlike the
-- older fast dialogue engine this does not rely on the RichText accelerator to
-- decide whether ClickContinue worked: it verifies an actual stage-token change
-- after every method, exactly like the analyzer.
function Inventory:RunDialogueTestAll(optionName, isComplete, timeout)
    optionName = tostring(optionName or "Option1")
    timeout = math.max(0.35, tonumber(timeout) or 2.5)
    if type(isComplete) ~= "function" then
        isComplete = function() return getDialogueGui() == nil end
    end
    if type(_rawGetConnections) ~= "function" then
        return false, "TEST ALL requires getconnections"
    end

    local startedAt = tick()
    local gui = getDialogueGui()
    if not gui then return false, "No DialogueGui open" end
    local unhide = testAllHideDialogue(gui)
    local passes = 0
    ...
    while tick() < deadline do
        ...
        if completeOk and complete then
            ...
            return true, string.format("%.3fs passes=%d method=%s", tick() - startedAt, passes, tostring(lastMethod))
        end
        ...
        local ok, method
        if sig ~= "" then
            ok, method = testAllOption1(gui, optionName)
        else
            ok, method = testAllClickContinue(gui)
        end
        lastMethod = method or lastMethod

        if ok then
            -- Analyzer R2 itself performs another pass after a successful 2088
            -- fire because DialogueGui can disappear asynchronously.  Mirror it.
            task.wait(0.02)
        else
            task.wait(0.02)
        end
