...
1. FINAL Zero-Delay route.
2. Hidden internal fallback.
3. Standard dialogue fallback.
...
| File | Responsibility |
|---|---|
| `Main.lua` | Entry point, module download/validation, init, diagnostics |
| `Config.lua` | Persistent configuration |
| `Webhook.lua` | Discord webhook notifications |
| `Movement.lua` | Movement, noclip, freeze, and camera helpers |
| `ServerHop.lua` | Server hopping/rejoin logic |
| `Inventory.lua` | Item counts, Auto Sell, Merchant zero-delay logic |
| `CombatFarm.lua` | Quest/NPC combat controller |
| `Farm.lua` | Main item farming lifecycle |
| `UI.lua` | Future Gold interface |
| `AutoPrestige.lua` | Prestige/story/leveling worker |
