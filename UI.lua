-- =====================
-- UI.lua (WHITE HUB V3 - BLACK & GOLD REWORK)
...
local UI_BUILD = "BLACK-GOLD-2026.09.14-R2"
local THEME = {
    BG0 = Color3.fromRGB(7, 7, 9),
    BG1 = Color3.fromRGB(11, 11, 14),
    PANEL = Color3.fromRGB(17, 17, 21),
    PANEL2 = Color3.fromRGB(23, 23, 29),
    PANEL3 = Color3.fromRGB(31, 31, 39),
    BORDER = Color3.fromRGB(103, 81, 28),
    BORDER_SOFT = Color3.fromRGB(70, 56, 20),
    GOLD = Color3.fromRGB(212, 175, 55),
    GOLD_LIGHT = Color3.fromRGB(255, 222, 125),
    GOLD_DARK = Color3.fromRGB(145, 112, 26),
    TEXT = Color3.fromRGB(246, 242, 230),
    MUTED = Color3.fromRGB(190, 169, 108),
    MUTED2 = Color3.fromRGB(137, 121, 78),
    SUCCESS = Color3.fromRGB(72, 150, 96),
    DANGER = Color3.fromRGB(146, 54, 54),
}

local function AddGradient(instance, a, b, rotation)
...
local function CreateCreditsPopup()
    ...
    t1.Text               = "WHITE HUB"
    t1.TextColor3         = THEME.GOLD_LIGHT
    t1.TextScaled         = true
...
local function MakeStyledDropdown(parent, labelText, options, callback)
    ...
    btnStroke.Color = THEME.BORDER_SOFT
...
function UI:Create()
    ...
    MainFrame.BackgroundColor3 = THEME.BG0
    ...
    AddGradient(MainFrame, THEME.BG0, THEME.BG1, 90)
    ...
    AccentLine.Name = "GoldAccentLine"
    AccentLine.Position = UDim2.new(0, 10, 0, 41)
    AccentLine.Size = UDim2.new(1, -20, 0, 1)
    AccentLine.BackgroundColor3 = THEME.GOLD
    ...
    Title.Text                   = "WHITE HUB  //  V3 GOLD"
    Title.TextColor3             = THEME.GOLD_LIGHT
    Title.TextSize               = 20
    ...
    local C_Stroke   = THEME.BORDER
    local C_StrokeAct= THEME.GOLD_LIGHT
    local C_Text     = THEME.TEXT
    ...
    for _, def in ipairs(tabDefs) do
        ...
        btn.MouseEnter:Connect(function()
            if not def.page.Visible then
                TweenService:Create(bs, TweenInfo.new(0.12), {Color = THEME.GOLD}):Play()
            end
        end)
        btn.MouseLeave:Connect(function()
            if not def.page.Visible then
                TweenService:Create(bs, TweenInfo.new(0.12), {Color = C_Stroke}):Play()
            end
        end)
    ...
    whBox.Focused:Connect(function()
        TweenService:Create(whStroke, TweenInfo.new(0.1), {Color=THEME.GOLD_LIGHT}):Play()
        whBox.BackgroundColor3 = THEME.PANEL2
    end)
    whBox.FocusLost:Connect(function()
        if _config then _config:Set("WebhookURL", whBox.Text) end
        TweenService:Create(whStroke, TweenInfo.new(0.1), {Color=THEME.BORDER}):Play()
        whBox.BackgroundColor3 = THEME.PANEL3
    end)
    ...
    resetBtn.MouseLeave:Connect(function()
        TweenService:Create(resetBtn, TweenInfo.new(0.15), {BackgroundColor3=THEME.GOLD_DARK}):Play()
    end)
    ...
    consoleHolder.BackgroundColor3 = THEME.BG1
    ...
    copyLogBtn.TextColor3 = Color3.fromRGB(15, 12, 6)
    ...
    clearLogBtn.TextColor3 = THEME.TEXT
    ...
    ToggleBtn.BackgroundColor3 = THEME.GOLD_DARK
    ToggleBtn.BorderSizePixel  = 0
    ToggleBtn.Position         = UDim2.new(0,10,1,-292)
    ToggleBtn.Size             = UDim2.new(0,156,0,40)
    ToggleBtn.Text             = "OPEN WHITE HUB // GOLD"
    ToggleBtn.TextColor3       = Color3.fromRGB(15, 12, 6)
    ToggleBtn.TextSize         = 13
    ToggleBtn.Font             = Enum.Font.GothamBold
    Instance.new("UICorner", ToggleBtn).CornerRadius = UDim.new(0,6)
    local tStroke = Instance.new("UIStroke", ToggleBtn)
    tStroke.Color     = THEME.GOLD
    tStroke.Thickness = 1.3
    AddGradient(ToggleBtn, THEME.GOLD_LIGHT, THEME.GOLD_DARK, 90)

    ToggleBtn.MouseEnter:Connect(function()
        TweenService:Create(tStroke, TweenInfo.new(0.15), {Color=THEME.GOLD_LIGHT}):Play()
    end)
    ToggleBtn.MouseLeave:Connect(function()
        TweenService:Create(tStroke, TweenInfo.new(0.15), {Color=THEME.BORDER}):Play()
    end)
